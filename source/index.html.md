...
- 本篇所列出的所有wss接口的baseurl为: **wss://wbs.mexc.com**
- Streams有单一原始 stream 或组合 stream
- 单一原始 streams 格式为 **/ws/<streamName>**
- 组合streams的URL格式为 **/stream?streams=<streamName1>/<streamName2>/<streamName3>**
- 订阅组合streams时，事件payload会以这样的格式封装: **{"stream":"<streamName>","data":<rawPayload>}**
- stream名称中所有交易对均为 **小写**
- 每个到 **[stream.mexc.com](http://stream.mexc.com/)** 的链接有效期不超过24小时，请妥善处理断线重连。
...
- **请求**
  ...
  { "method": "SUBSCRIBE", "params": [ "spot@public.aggTrade@btcusdt","spot@public.depth@btc_usdt" ], "id": 1 }

待确认：{ "method": "SUBSCRIBE", "params": [ "spot@private.order" ], "id": 1 }
...
{
  "e": "aggTrade",  // 事件类型
  "E": 123456789,   // 事件时间，时间戳
  "s": "MXBTC",    // 交易对
  "a": 12345,       // 归集交易ID【新生成ID】
  "p": "0.001",     // 成交价格【平均成交价格】
  "q": "100",       // 成交数量
  "f": 100,         // 被归集的首个交易ID【trade id 】
  "l": 105,         // 被归集的末次交易ID【trade id 】
  "T": 123456785,   // 成交时间
  "m": true,        // 买方是否是做市方。如true，则此次成交是一个主动卖出单，否则是一个主动买入单。【买方是否maker】
}
```

归集交易 stream 推送交易信息，是对单一订单的集合。

**Stream 名称:** `<symbol>@aggTrade` **Update Speed:** 实时
...
```
{
  ...
  "a": 50,          // 卖方的订单ID
  "T": 123456785,   // 成交时间
  "m": true        // 买方是否是做市方。如true，则此次成交是一个主动卖出单，否则是一个主动买入单。待支持
}
```

**Stream Name:** `<symbol>@trade -> spot@public.deals@<symbol>`
...
```
{
  ...
  "k": {
    ...
    "s": "MXBTC",  // 交易对
    "i": "1m",      // K线间隔
    "f": 100,       // 这根K线期间第一笔成交ID【待确认】不支持
    "L": 200,       // 这根K线期间末一笔成交ID【待确认】不支持
    "o": "0.0010",  // 这根K线期间第一笔成交价
    "c": "0.0020",  // 这根K线期间末一笔成交价
    "h": "0.0025",  // 这根K线期间最高成交价
    "l": "0.0015",  // 这根K线期间最低成交价
    "v": "1000",    // 这根K线期间成交量
    "n": 100,       // 这根K线期间成交笔数 待支持
    "x": false,     // 这根K线是否完结(是否已经开始下一根K线)
    "q": "1.0000",  // 这根K线期间成交额
    "V": "500",     // 主动买入的成交量【买方是taker的成交量】待支持
    "Q": "0.500",   // 主动买入的成交额【买方是taker的成交额】待支持
  }
}
```
 ...
 **Stream Name:** `<symbol>@kline_<interval> -> spot@public.kline@<symbol>@<interval>`

**Update Speed:** 2000ms → 准实时

**K线图间隔参数:**

m -> 分钟; h -> 小时; d -> 天; w -> 周; M -> 月【我们有的显示】未支持所有间隔
...
## 按 Symbol 的精简Ticker （可能暂时不好支持，待cache重构）
...
## 全市场所有Symbol的精简Ticker （可能暂时不好支持，待cache重构）
...
## 按Symbol的完整Ticker （可能暂时不好支持，待cache重构。或定时任务推）
...
```
{
  ...
  "w": "0.0018",      // 平均价格
  "x": "0.0009",      // 整整24小时之前，向前数的最后一次成交价格 （不懂。。）
  "c": "0.0025",      // 最新成交价格 
  "Q": "10",          // 最新成交交易的成交量 （暂不支持）
  "b": "0.0024",      // 目前最高买单价
  "B": "10",          // 目前最高买单价的挂单量 （暂不支持）
  "a": "0.0026",      // 目前最低卖单价
  "A": "100",         // 目前最低卖单价的挂单量 （暂不支持）
  "o": "0.0010",      // 整整24小时前，向后数的第一次成交价格 （不懂。。）
  "h": "0.0025",      // 24小时内最高成交价
  "l": "0.0010",      // 24小时内最低成交价
  "v": "10000",       // 24小时内成交量
  "q": "18",          // 24小时内成交额
  "O": 0,             // 统计开始时间 （？）
  "C": 86400000,      // 统计结束时间 （？）
  "F": 0,             // 24小时内第一笔成交交易ID【没有】
  "L": 18150,         // 24小时内最后一笔成交交易ID【没有】
  "n": 18151          // 24小时内成交数 （待支持）
}
```
...
## 全市场所有交易对的完整Ticker（可能暂时不好支持，待cache重构）
...
每秒或每100毫秒推送有限档深度信息。levels表示几档买卖单信息, 可选 5/10/20档【最多20档吗？】
...
**Stream Name:** `<symbol>@depth` 或 `<symbol>@depth@100ms -> spot@public.depth@<symbol>`

**Update Speed:** 1000ms 或 100ms

## 如何正确在本地维护一个orderbook副本

1. 订阅 **[wss://stream.mexc.com:9443/ws/mxbtc@depth](wss://stream.mexc.com:9443/ws/mxbtc@depth)**
...
- 本篇所列出API接口的base url : **[https://api.mexc.com](https://api.mexc.com/)**
...
- websocket接口的baseurl: **[wss://stream.mexc.com:9443](wss://stream.mexc.com:9443/)**
- U订阅账户数据流的stream名称为 **/ws/<listenKey>** 或 **/stream?streams=<listenKey>**
- 每个链接有效期不超过24小时，请妥善处理断线重连。
- 账户数据流的消息不保证严格时间序; **请使用 E 字段进行排序**

## Listen Key(现货账户) listenKey是否区分子账户
...
```
{}
PUT /api/v3/spot/userDataStream
```
...
```
{}
DELETE /api/v3/spot/userDataStream
```
...
POST /api/v3/margin/userDataStream/isolated
```
...
```
{}
PUT /api/v3/margin/userDataStream/isolated
```
...
```
{}
DELETE /api/v3/margin/userDataStream/isolated
```
...
首次和定时推全部有仓位持仓；后续持仓下有一个成交改变其中的一个持仓，那么持仓变更只推这一个。
...
```
{
  ...
  "B": [                          // 余额
    {
   “ct”: 1499405658658,            // 持仓创建时间
   “ut”: 1499405658658,            // 最近一次持仓更新时间
   "s": "ETHBTC",                 // 交易对
   “m”: "isolated",                    // 保证金模式（全仓/逐仓）
   "q": "1.00000000",             // 持仓数量
   “ba”: “8.00000000",             // 交易货币余额
   “qa”: “3.00000000",             // 计价货币余额
   “c”: “ETH”,             // 持仓数量的币种
   “aq": "1.00000000",             // 可平仓数量
   “p": "1.00000000",             // 开仓平均价
   “u": "1.00000000",             // 未实现收益
   “ur”: "1.00000000",             // 未实现收益率
   "l": “10”,             // 杠杆倍数
   "lp”: “10”,             // 预估强评价
   “m”: “10”,             // 保证金余额
   “mr”: “10”,             // 保证金率
...
请查阅文档[公开API参数](https://mexc-docs.github.io/apidocs/spot/cn/#public-api-definitions)以及以下文档，以获取相关的枚举定义。
